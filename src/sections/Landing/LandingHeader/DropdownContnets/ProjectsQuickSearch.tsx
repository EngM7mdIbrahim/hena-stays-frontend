...
import { GetAllProjectsQuery } from '@commonTypes'
...
export interface ProjectsQuickSearchProps {
  className?: string
}

function ProjectsQuickSearch({ className }: ProjectsQuickSearchProps) {
  const isMobile = useMediaQuery('(max-width: 768px)')
  const t = useTranslations()
  const { encodeFilters } = useUpdateSearchParams<
    Pick<GetAllProjectsQuery, 'filter' | 'text'>
  >({ activate: false })
    ...
    <Box className={cn('relative z-20', className)}>
      ...
      <form
        onSubmit={(e) => {
          e.preventDefault()
          router.push(
            `${navigationLinks.projects.allProjects}?${encodeFilters(filters)}`
          )
        }}
      ...
      >
        <Box className='flex flex-wrap justify-between gap-4'>
          <SearchField
            size='lg'
            className={cn('outline-none', isMobile ? 'w-full' : 'w-[350px]')}
...
export default ProjectsQuickSearch
